...
AMY_AUDIO_API void InitMicArrayProc(MicArrayProcInst * self)
{
	...
	self->agc_config = new Agc_Outer_Config[sizeof(Agc_Outer_Config)];
	self->agc_config->sample_frequency = kSampleRate;
	self->agc_config->frame_len = kFrameLen;
	self->agc_config->min_mic_level = 0;
	self->agc_config->max_mic_level = 255;
	self->agc_config->in_mic_level = 0;
	self->agc_config->out_mic_level = 0;
	self->agc_config->compressionGaindB = 20;
	self->agc_config->targetLevelDbfs = 6;
	self->agc_config->limiterEnable = 1;
	self->agc_config->agc_mode = kAgcModeAdaptiveDigital;
	self->agc_config->agc_inst = NULL;

	agc_Create(self->agc_config);
	agc_Init(self->agc_config);
	agc_Configure(self->agc_config);
...
AMY_AUDIO_API void SetParasMicArrayProc(MicArrayProcInst * self, int win_type)
{
	...
	ScaleVector(self->snr_est->smooth_win_of_sp->data(), self->snr_est->smooth_win_of_sp->data(), self->snr_est->smoothed_win_len_of_sp, 1.0f / norm);
	...
	ScaleVector(self->snr_est->smoothed_local_win->data(), self->snr_est->smoothed_local_win->data(), self->snr_est->smoothed_local_win_len, 1.0f / norm);
	...
	ScaleVector(self->snr_est->smoothed_global_win->data(), self->snr_est->smoothed_global_win->data(), self->snr_est->smoothed_global_win_len, 1.0f / norm);
	...
	ScaleVector(self->vadInst->smooth_win_of_sp->data(), self->vadInst->smooth_win_of_sp->data(), self->vadInst->smoothed_win_len_of_sp, 1.0f / norm);
...
AMY_AUDIO_API void ProcCoreMicArrayProcSingleOut(MicArrayProcInst * self, float ** in_multi_chs_data, float * out_single_ch_data)
{
	...
	for (i = 0; i < kNumChannels; i++)
	{
		InputBufferWrite(in_multi_chs_data[i], self->buf_rw_status[i].in_buffer.data(), kFrameLen,
			self->buf_rw_status[i].read_pos, self->buf_rw_status[i].write_pos, self->buf_rw_status[i].rw_wrap, kBufferLen);
	}
		...
		for (i = 0; i < kNumChannels; i++)
		{
			...
			if (self->frame_num >= 5)
			{
				...
				tmp_fd_chs.col(i) = self->doa_core->sp_smoothing * tmp_fd_chs_prev.col(i) +
					(1 - self->doa_core->sp_smoothing) * tmp_fd_chs.col(i).cwiseProduct(self->gain.col(i));
...
AMY_AUDIO_API void ProcCoreMicArrayProcMultiOut(MicArrayProcInst * self, float ** in_multi_chs_data, float ** out_multi_chs_data)
{
	...
	*/
	...
	while (first_frame_in_block < kFrameLen)
	{
		for (i = 0; i < kNumChannels; i++)
		{
			...
			EstimateSnrAndGainEtc(&(self->block_tf[kBlocks].in_fd_block),
				...
				&self->prior_snr, &self->post_snr, &self->gain, &self->speech_prob, self->frame_num, i);
	...
	for (j = 0; j < kNumChannels; j++)
	{
		...
		OutputSamplesAndUpdateBuffer(out_multi_chs_data[j],
			self->buf_rw_status[j].out_buffer.data(), kFrameLen, kBufferLen, kBlockInitialDelay);
	}
...
AMY_AUDIO_API void EstimateSnrAndGainEtc(MatrixXf *xf, VaNEstInst* psnr, MatrixXf *noise_est, MatrixXf *recur_avg_sp_est,
	MatrixXf *prior_snr, MatrixXf *post_snr, MatrixXf *gain, MatrixXf *speech_prob, int frame_seq, int ch_seq)
{
	...
	*/
	...
	if (frame_seq >= NIS)
	{
		...
		ComputeDotQuotientOf2Vecs(recur_avg_sp_est->col(ch_seq).data(), min_Ef.col(ch_seq).data(), likelihood_ratio.data(), kNumFreqs);
		...
		CopyValsOfVecsToOther(SpeechPresenceProb.col(ch_seq).data(), SpeechPresenceProb_Prev.col(ch_seq).data(), kNumFreqs);
		...
		ComputePFrameAndSnrPeak(&P_Frame, &RecurAvgPriorSNR_Peak, RecurAvgPriorSNR_PeakPrev, AvgRecurAvgPriorSNR,
			AvgRecurAvgPriorSNRPrev, psnr->sigma_min, psnr->sigma_max, psnr->sigma_pmin, psnr->sigma_pmax, ch_seq);
		...
		ComputePriorSNRCurr(prior_snr->col(ch_seq).data(), GainH1_Prev.col(ch_seq).data(),
			PostSNR_Prev.col(ch_seq).data(), post_snr->col(ch_seq).data(), psnr->compromising, kNumFreqs);
...
AMY_AUDIO_API void ComputePFrameAndSnrPeak(float *p_frame, float *snr_peak, float *snr_peak_prev, float seg_avg_snr,
	float *seg_avg_snr_prev, float snr_min, float snr_max, float snr_pmin, float snr_pmax, int ch_seq)
{
	...
	float *voice_prob, int frame_seq, int vad_mode)
{
	...
	else
	{
		...
		case 2:
			...
			bkg_noise_energy_entropy_ratio = ComputeLogEnergyEntropyRatio(avg_nf, vad->frequency_start_ind, vad->frequency_end_ind, vad->alpha_log_sp);
			/* compute energy entropy ratio with certain frequency range for current frame */
			instant_energy_entropy_ratio = ComputeLogEnergyEntropyRatio(avg_sf, vad->frequency_start_ind, vad->frequency_end_ind, vad->alpha_log_sp);
			vad->is_speech_frame = instant_energy_entropy_ratio > bkg_noise_energy_entropy_ratio * vad->beyond_ratio_ee ?
				true : false;
			break;
		default:
...
AMY_AUDIO_API void ValidVoiceSegDecisionAlongFrames(VoiceDetInst *vad, int frame_seq)
{
	...
	if (!vad->is_speech_triggered)
	{
		...
		valid_nums = frame_seq < vad->non_speech_to_speech_frame_len ? frame_seq + 1 : vad->non_speech_to_speech_frame_len;
	...
	{
		valid_nums = frame_seq < vad->speech_to_non_speech_frame_len ? frame_seq + 1 : vad->speech_to_non_speech_frame_len;
...
AMY_AUDIO_API void DoaEstimationCore(MatrixXcf *chs_com_sp, DoaInst* pdoa, ArrayProps *mic_info, int doa_mode)
{
	...
	*/
	...
	static int scan_angle_num = mic_info->is_linear ? (int((pdoa->coarse_ang_max1 - pdoa->coarse_ang_start) /
		pdoa->coarse_ang_span) + 1) : (int((pdoa->coarse_ang_max2 - pdoa->coarse_ang_start) /
	...
	for (k1 = 0; k1 < scan_angle_num; k1++)
	{
		...
		for (k2 = pdoa->frequency_start_ind; k2 <= pdoa->frequency_end_ind; k2++)
		{
			...
			case 0: 
				Pf_out(k1) += ComputeOutSpatialSpectrumCbf(array_steer_vecter.data(), freq_wise_complex_spectrum.data(), kNumChannels);
				break;
			/* minimum variance distortionless response beamforming */
			case 1:
				Pf_out(k1) += ComputeOutSpatialSpectrumMvdr(array_steer_vecter.data(), freq_wise_complex_spectrum.data(), kNumChannels, pdoa->loading_factor);
				break;
...
AMY_AUDIO_API float ComputeOutSpatialSpectrumCbf(const Complexf * steer_vcetor, const Complexf * In_FreqSpectrum, int len)
{
	float tmp = 0.0f;
	Complexf ctmp(0.0f, 0.0f), conj_steer_vector(0.0f, 0.0f);
...
AMY_AUDIO_API float ComputeOutSpatialSpectrumMvdr(const Complexf * steer_vcetor, const Complexf * In_FreqSpectrum, int len, float load_factor)
{
	float tmp = 0.0f;
	Complexf ctmp(0.0f, 0.0f), conj_steer_vector(0.0f, 0.0f);
...
AMY_AUDIO_API void BfCore(MatrixXcf *in_chs_com_sp, MatrixXcf *out_chs_com_sp, bfInst *pbf, ArrayProps *mic_info, int bf_mode)
{
	...
	for (k = 0; k < kNumFreqs; k++)
	{
		...
		// conventional beamforming
		BfPhaseCompensation(array_steer_vecter.data(), freq_wise_complex_spectrum.data(), freq_wise_complex_spectrum.data(), kNumChannels);
...
AMY_AUDIO_API void PostFilterCore(MatrixXcf *in_chs_com_sp, MatrixXcf *single_out_ch_com_sp, PfInst *ppf, int cn_mode)
{
	...
	for (k1 = 0; k1 < kNumChannels - 1; k1++)
		for (k2 = k1 + 1; k2 < kNumChannels; k2++)
		{
			ComputePowerDensity(inter_sp_prev.col(k).data(), inter_sp.col(k).data(),
				in_chs_com_sp->col(k1).data(), in_chs_com_sp->col(k2).data(), ppf->smooth_alpha, kNumFreqs);
	...
	if (cn_mode == 0)
	{
		GetCwiseMaxValOfVecAndScalar(post_filter_coeff.data(), ppf->min_postfilter_coeff, post_filter_coeff.data(), kNumFreqs);
	}
	else if (cn_mode == 1)
	{
		GetCwiseMaxValOf2Vecs(post_filter_coeff.data(), ppf->min_postfilter_coeff_cn->data(), post_filter_coeff.data(), kNumFreqs);
	}
...
//	ApplyPostFilterCoeff(single_out_ch_com_sp->data(), single_out_ch_com_sp->data(), post_filter_coeff.data(), kNumFreqs);
	...
	ConstructComSpFromEsp_Psp(single_out_ch_com_sp->data(), avg_square_sp.col(0).data(), angle_sp.col(0).data(), kNumFreqs);
	post_filter_coeff.setConstant(1.0f);
	ApplyPostFilterCoeff(single_out_ch_com_sp->data(), single_out_ch_com_sp->data(), post_filter_coeff.data(), kNumFreqs);
}

AMY_AUDIO_API void ComputePowerDensity(float * power_density_prev, float * power_density,
	Complexf * ch1_comp_spectrum, Complexf * ch2_comp_spectrum, float smooth_factor, int len)
{
	int i = 0;
...
AMY_AUDIO_API void PostFilterGetFilterCoeffMin(float * min_filter_coeff, int low_ind, int high_ind, int len, float slope)
{
	int i = 0;
...
AMY_AUDIO_API void ComputeAverageComSpectrum(Complexf * src_com_spectrum, Complexf * dst_com_spectrum, int chs, int freqs)
{
	int i = 0, j = 0, src_chs_ind = 0;
	for (i = 0; i < freqs; i++)
	{
...
{
	agc_Free(self->agc_config);
	delete self->agc_config;
