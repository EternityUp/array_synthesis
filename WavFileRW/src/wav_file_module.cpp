...
// Does't take ownership of the file handle and won't close it.
...
WavReader::WavReader(const std::string &filename)
    ...
    if (file_handle_ == NULL) {
        std::cout << "Could not open wav file for reading.";
    }
...
WavWriter::WavWriter(const std::string &filename, int sample_rate,
    ...
    if (file_handle_ == NULL) {
        std::cout << "Could not open wav file for writing.";
    }
    assert(CheckWavParameters(num_channels_, sample_rate_, kWavFormat,
...
#error "Need to convert samples to little-endian when writing to WAV file"
    ...
    assert(num_samples_ >= written);  // detect size_t overflow
}
...
void WavWriter::Close() {
    ...
    WriteWavHeader(header, num_channels_, sample_rate_, kWavFormat,
                   kBytesPerSample, num_samples_);
    assert(1 == fwrite(header, kWavHeaderSize, 1, file_handle_));
    assert(0 == fclose(file_handle_));
    file_handle_ = nullptr;
}
